...
export default function App() {
  const {toggleMode, Modedark} = UseMode();
  return (
    <div className={"w-screen h-screen " + (Modedark? "dark": "")}>
      <div className="relative size-full dark:bg-slate-900 bg-slate-100 transition-colors ease-in duration-500">
        <div className="absolute z-0 w-full h-[300px] bg-purple-800 ">
          <BackGroundImage modeActive={Modedark}/>
        </div>
        <main className="absolute z-10 w-full px-8 h-full flex justify-center">
          <div className="sm:w-[540px] w-full h-full pt-16 flex flex-col">
            <Navbar toggle={toggleMode} modeActive={Modedark}/>
            <PanelTasks/>
          </div>
        </main>
      </div>
